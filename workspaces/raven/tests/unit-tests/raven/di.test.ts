...
describe("Raven dependency injection functions", () => {
  test("UseController registers type", () => {
    ...
    verify(
      dependencyContainerMock.register(Raven.ControllersSymbol, EmptyController)
    ).once();
  });
  ...
  test("UseModel registers Model constructor", () => {
    ...
    verify(
      dependencyContainerMock.registerInstance(Raven.ModelsSymbol, EmptyModel)
    ).once();
  });
  ...
  test("UseMiddleware registers class", () => {
    ...
    verify(
      dependencyContainerMock.register(anything(), EmptyMiddleware)
    ).once();
  });
